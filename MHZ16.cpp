    ...
    , uart_evt_queue(NULL)
{
    ...
    {
        .callback = warmup_timer_cb,
        .arg = this, 
        .dispatch_method = ESP_TIMER_TASK,
        .name = "MHZ16 warmup timer",
...
void MHZ16::set_self_calibration(bool enable)
{
    ...
    assemble_tx_packet(packet, CMD_SET_SELF_CALIBRATE,
      {
        enable ? static_cast<uint8_t>(0xA0) : static_cast<uint8_t>(0x00), 
        0, 
        0,
        0,
        0
...
void MHZ16::calibrate_span_point(uint16_t span_point)
{
    ...
    assemble_tx_packet(packet, CMD_CAL_SPAN_POINT,
      {
        static_cast<uint8_t>(span_point / 256), 
        static_cast<uint8_t>(span_point % 256), 
        0,
        0,
        0
...
void MHZ16::set_detection_range(uint32_t range)
{
    ...
    assemble_tx_packet(packet, CMD_SET_DETECTION_RANGE, 
        {
            0, 
            static_cast<uint8_t>((range & 0xFF000000) >> 3 * 8), 
            static_cast<uint8_t>((range & 0x00FF0000) >> 2 * 8),
            static_cast<uint8_t>((range & 0x0000FF00) >> 1 * 8),
            static_cast<uint8_t>((range & 0x000000FF) >> 0 * 8)
        }
    );
    uart_write_bytes(cfg.uart_peripheral, packet.data(), PACKET_SZ);
    vTaskDelay(100/portTICK_PERIOD_MS); //allow some time for MHZ16 to execute command (it is really slow, documentation is poor on this)
